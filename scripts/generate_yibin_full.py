"""
Generate Yibin landslide FULL video demo data.
Covers the entire 105s video with adaptive frame skipping disabled,
so both early precursor rockfalls and later major collapse are captured.
"""
...
# Force no adaptive skipping + disable deep idle — capture every motion event
os.environ['SKIP_IDLE'] = '1'
os.environ['SKIP_ACTIVE'] = '1'
os.environ['SKIP_CRITICAL'] = '1'
os.environ['DEEP_IDLE_ENABLED'] = 'false'
...
def main():
    ...
    # Run on FULL video with stride=3
    # 3160 frames / 3 ≈ 1053 inference frames
    # Load site-specific ROI polygon if configured
    polygon = None
    from rockfall.site_config import get_site_by_id as _get_site
    _site = _get_site("yibin_s1")
    if _site and _site.roi_polygon:
        polygon = np.array(_site.roi_polygon, dtype=np.int32)
        print(f"    Using site ROI polygon: {len(polygon)} vertices")
    else:
        print(f"    Using default ROI polygon")

    print(f"\n[2] Running FULL video detection (stride=3, no adaptive skip)...")
    t0 = time.time()
    ...
    # Level distribution
    level_counts = Counter(fr.get("alert_level", "green") for fr in detections)
    print(f"    Levels: R={level_counts['red']} O={level_counts['orange']} "
          f"Y={level_counts['yellow']} B={level_counts['blue']} G={level_counts['green']}")

    # Show alert escalation timeline
    print(f"\n[3] Alert timeline (level transitions):")
    prev_lvl = "green"
    for fr in alert_frames:
        ...
        if lvl != prev_lvl:
            ...
            emoji = {"red": "🔴", "orange": "🟠", "yellow": "🟡", "blue": "🔵"}.get(lvl, "⚪")
            print(f"    {ts:>6.1f}s | {prev_lvl:>6} → {lvl:>6} {emoji} | "
                  f"conf={conf:.4f}, boxes={len(boxes)}")
            prev_lvl = lvl

    # Select key frames: top 10 of each alert level (enough for good thumbnail strip)
    ...
    for lvl in ["red", "orange", "yellow", "blue"]:
        ...
        selected = lvl_frames[:10]
        key_frames.extend(selected)
        print(f"    {lvl}: {len(lvl_frames)} alert frames, selected top {len(selected)}")

    # Pad with green frames if total < 40 (for a rich demo)
    if len(key_frames) < 40:
        green = [fr for fr in detections if fr.get("alert_level") == "green"][
            :40 - len(key_frames)
        ]
        key_frames.extend(green)

    # Save thumbnails
    print(f"\n[5] Saving {len(key_frames)} key frame thumbnails...")
    saved_frames = []
    for i, fr in enumerate(key_frames):
        frame_idx = fr["frame"]
        orig_path = _THIS_DIR / "data" / "results" / f"stream_{frame_idx:06d}.jpg"
        lvl = fr.get("alert_level", "green")
        thumb_name = f"{i:03d}_{lvl}.jpg"
    ...
    print(f"    summary.json written ({len(saved_frames)} key frames)")

    # Write result.json (track data for Kalman charts)
    track_frames = []
    ...
    print(f"\n{'=' * 60}")
    print(f"DONE! Full video demo data: {OUT_DIR}")
    print(f"Thumbnails: {len(list(FRAMES_DIR.glob('*.jpg')))} files")
